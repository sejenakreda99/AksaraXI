...
function useMenyimakContent(chapterId: string) {
        ...
        async function fetchContent() {
            ...
            try {
                ...
                if (docSnap.exists() && docSnap.data().menyimak) {
                    setContent(docSnap.data().menyimak as MenyimakContentData);
                }
            } catch (error) {
...
// --- Activity Components ---
function Kegiatan1({ content, chapterId }: { content: MenyimakContentData, chapterId: string }) {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            await setDoc(submissionRef, { kegiatan1: { answers }, lastSubmitted: serverTimestamp(), studentId: user.uid, chapterId, activity: 'menyimak' }, { merge: true });
            toast({ title: "Berhasil!", description: "Jawaban Anda telah berhasil disimpan." });
        } catch (error) {
    ...
    return (
        <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <Button type="submit" className="w-full mt-6" disabled={isSubmitting}>
                ...
                {isSubmitting ? 'Mengirim...' : 'Simpan & Kirim Jawaban Kegiatan 1'}
            </Button>
        </form>
    )
}

function Kegiatan2({ content, chapterId }: { content: MenyimakContentData, chapterId: string }) {
    ...
    const handleActivity2AnswerChange = (question: string, value: string) => {
     ...
     const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const submissionRef = doc(db, 'submissions', `${user.uid}_${chapterId}_menyimak`);
            await setDoc(submissionRef, { kegiatan2: { activity2Answers }, lastSubmitted: serverTimestamp(), studentId: user.uid, chapterId, activity: 'menyimak' }, { merge: true });
            toast({ title: "Berhasil!", description: "Jawaban Anda telah berhasil disimpan." });
        } catch (error) {
            toast({ variant: "destructive", title: "Gagal Menyimpan", description: "Terjadi kesalahan." });
        } finally {
    ...
    return (
        <form className="space-y-6" onSubmit={handleSubmit}>
                    ...
                    <div className="space-y-4">
                        {(content.activity2Questions || []).map((q, i) => (
                            <div key={i}>
                                ...
                                <Textarea id={`activity2-q${i}`} className="mt-2" placeholder="Tuliskan jawaban Anda di sini..." onChange={e => handleActivity2AnswerChange(q, e.target.value)} value={activity2Answers[q] || ''} />
                            </div>
                        ))}
                    </div>
                    <Separator />
            ...
            <Button type="submit" className="w-full mt-6" disabled={isSubmitting}>
                {isSubmitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Send className="mr-2 h-4 w-4" />}
                {isSubmitting ? 'Mengirim...' : 'Simpan & Kirim Jawaban Kegiatan 2'}
            </Button>
        </form>
    );
}

function Latihan({ content, chapterId }: { content: MenyimakContentData, chapterId: string }) {
    const [latihanAnswers, setLatihanAnswers] = useState<Record<string, { choice: 'benar' | 'salah' | ''; analysis: string }>>({});
    const [isSubmitting, setIsSubmitting] = useState(false);
    const { toast } = useToast();
    const [user] = useAuthState(auth);
    ...
    const handleLatihanAnswerChange = (no: string, type: 'choice' | 'analysis', value: string) => {
     ...
     const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const submissionRef = doc(db, 'submissions', `${user.uid}_${chapterId}_menyimak`);
            await setDoc(submissionRef, { latihan: { latihanAnswers }, lastSubmitted: serverTimestamp(), studentId: user.uid, chapterId, activity: 'menyimak' }, { merge: true });
            toast({ title: "Berhasil!", description: "Jawaban Anda telah berhasil disimpan." });
        } catch (error) {
            toast({ variant: "destructive", title: "Gagal Menyimpan", description: "Terjadi kesalahan." });
        } finally {
    ...
    return (
        <form className="space-y-6" onSubmit={handleSubmit}>
            <Card>
             ...
             <Button type="submit" className="w-full mt-6" disabled={isSubmitting}>
                {isSubmitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Send className="mr-2 h-4 w-4" />}
                {isSubmitting ? 'Mengirim...' : 'Simpan & Kirim Jawaban Latihan'}
            </Button>
        </form>
    );
}
...
export default function MenyimakSiswaPage() {
    ...
    const { content, loading } = useMenyimakContent(chapterId);

    const getPageTitle = () => {
        if (slug !== 'menyimak') return 'Materi'; // Fallback
        switch (activity) {
    ...
    const renderContent = () => {
        ...
        switch (activity) {
            case '1': return <Kegiatan1 content={content} chapterId={chapterId} />;
            case '2': return <Kegiatan2 content={content} chapterId={chapterId} />;
            case '3': return <Latihan content={content} chapterId={chapterId} />;
            default:
                // This should not happen if navigated from the hub page, but as a fallback:
                return <Card><CardHeader><CardTitle>Pilih Kegiatan</CardTitle></CardHeader><CardContent><p>Silakan kembali dan pilih sebuah kegiatan untuk dimulai.</p></CardContent></Card>;
        }
    };

    // This is the "Hub Page" for a specific slug like 'menyimak' or 'membaca'
    if (!activity) {
         const activityCards = {
            menyimak: [
                { title: "Kegiatan 1: Menganalisis Teks Deskripsi", kegiatan: "1", icon: Youtube, description: "Menganalisis video dan menilai kebenaran pernyataan.", disabled: false },
                { title: "Kegiatan 2: Mengevaluasi Gagasan", kegiatan: "2", icon: ClipboardCheck, description: "Menjawab pertanyaan analisis dan membandingkan video.", disabled: false },
                { title: "Latihan", kegiatan: "3", icon: ClipboardList, description: "Menganalisis video baru dan menilai gagasan narator.", disabled: false }
            ],
            membaca: [ { title: "Kegiatan 1: Menganalisis Teks", kegiatan: "1", icon: FileText, description: "Baca dan pahami teks deskripsi.", disabled: false } ]
        };
                    ...
                    <main className="flex-1 p-4 md:p-8">
                        <div className="max-w-4xl mx-auto">
                            {currentActivities.length > 0 ? (
                                <Card>
                                    <CardHeader><CardTitle>Pilih Kegiatan</CardTitle><CardDescription>Pilih kegiatan yang ingin Anda mulai dari tahap &quot;{slug}&quot; ini.</CardDescription></CardHeader>
                                    <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {currentActivities.map((act) => {
                                            const cardContent = (<Card className={cn("w-full p-6 flex flex-col items-center justify-center text-center hover:bg-slate-100 transition-colors hover:shadow-lg rounded-xl", act.disabled && "bg-slate-50 opacity-60 cursor-not-allowed")}>
                                            ...
                                            if(act.disabled) return <div key={act.title} className="flex">{cardContent}</div>
                                            const href = `/student/materi/${chapterId}/${slug}?kegiatan=${act.kegiatan}`;
                                            return <Link href={href} key={act.title} className="flex">{cardContent}</Link>
                                        })}
                                    </CardContent>
                                </Card>
                            ) : (
                                <Card><CardHeader><CardTitle>Segera Hadir</CardTitle></CardHeader><CardContent><p>Konten untuk bagian &quot;{slug}&quot; belum tersedia.</p></CardContent></Card>
                            )}
                        </div>
                    </main>
                </div>
    ...
    // This is the page for a specific activity
    return (
        <AuthenticatedLayout>
            <div className="flex flex-col h-full">
                <header className="bg-card border-b p-4 md:p-6">
                    <div className="max-w-4xl mx-auto">
                        <Button asChild variant="outline" size="sm" className="mb-4">
                            <Link href={`/student/materi/${chapterId}/${slug}`}>
                                <ArrowLeft className="mr-2 h-4 w-4" />
